...
public final class FieldCheckpoint {
  ...
  public static void process(
    ...
    if (s.confiscate == 2 && level.hasChunkAt(storagePos)) {
      var block = level.getBlockEntity(storagePos);
      if (block != null) storage = block.getCapability(ForgeCapabilities.ITEM_HANDLER, s.storageFace.getOpposite()).orElse(null);
    }
