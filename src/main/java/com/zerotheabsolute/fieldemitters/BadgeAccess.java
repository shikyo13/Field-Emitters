...
public final class BadgeAccess extends SavedData {
  ...
  public static BadgeAccess get(ServerLevel level) {
    return level
        ...
        .getDataStorage()
        .computeIfAbsent(
            new SavedData.Factory<>(BadgeAccess::new, BadgeAccess::load, null), "fieldemitters_badges");
  }

  public static BadgeAccess load(CompoundTag tag, HolderLookup.Provider registries) {
  ...
  public void issue(ItemStack stack, UUID issuer, String group, UUID player) {
    var previous = stack.get(DataComponents.CUSTOM_DATA);
    if (previous != null && previous.copyTag().hasUUID("BadgeId")) {
      var old = issued.get(previous.copyTag().getUUID("BadgeId"));
    ...
    stack.set(DataComponents.CUSTOM_DATA, CustomData.of(display));
    stack.set(
  ...
  public static List<ItemStack> carried(Player player) {
    ...
    if (net.fabricmc.loader.api.FabricLoader.getInstance().isModLoaded("trinkets"))
      result.addAll(TrinketsBridge.equipped(player));
    return result;
  }
