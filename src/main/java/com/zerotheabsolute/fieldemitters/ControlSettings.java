...
public final class ControlSettings {
  public static final float MAX_DAMAGE = 1000;
  public static final int MIN_DAMAGE_INTERVAL = 10;
  public static final int MAX_DAMAGE_INTERVAL = 200;
  public static final int DEFAULT_DAMAGE_INTERVAL = 20;

  public com.zeromods.core.animation.SphereFormation projection =
  ...
  public int linkFormationTicks(int distance) {
    return com.zeromods.core.animation.PlanarProjection.selected(formation)
        ? com.zeromods.core.animation.PlanarProjection.DURATION_TICKS
        : distance * 2;
  }
  ...
  public EntityFilter barrier = new EntityFilter(),
      ...
      damage = new EntityFilter();
  public final com.zeromods.core.filter.DirectionalRules<Direction, EntityFilter>
      barrierDirections = new com.zeromods.core.filter.DirectionalRules<>();
  public final com.zeromods.core.filter.DirectionalRules<Direction, EntityFilter> sensorDirections =
      new com.zeromods.core.filter.DirectionalRules<>();
  public final com.zeromods.core.filter.DirectionalRules<Direction, EntityFilter> damageDirections =
      new com.zeromods.core.filter.DirectionalRules<>();
  public boolean damageEnabled = false, fizzleEffects = true;
  public float damageAmount = 2;
  public int damageInterval = DEFAULT_DAMAGE_INTERVAL;
  public int pattern = 0, formation = 0, particleColor = 0xFF55DD;
  public boolean customAccent = false;

  public int accentColor(int fieldColor) {
    return customAccent ? particleColor : fieldColor;
  }

  public boolean sounds = true, powerSounds = true, impactSounds = true, damageSounds = true;
  public int soundStyle = 0;

  public EntityFilter damage(Direction movement) {
    return damageDirections.resolve(movement, damage);
  }
  ...
  public EntityFilter barrier(Direction movement) {
    return barrierDirections.resolve(movement, barrier);
  }

  public EntityFilter sensor(Direction movement) {
    return sensorDirections.resolve(movement, sensor);
  }
  ...
  public java.util.List<EntityFilter> filters() {
    ...
    result.add(sensor);
    result.add(damage);
    result.addAll(barrierDirections.overrides().values());
    result.addAll(sensorDirections.overrides().values());
    result.addAll(damageDirections.overrides().values());
    result.add(checkpoint.players);
    result.add(checkpoint.items);
    result.addAll(checkpoint.directions.overrides().values());
    return result;
  }
  ...
  public CompoundTag save() {
    ...
    damageDirections
        .overrides()
        .forEach((direction, rule) -> damageRules.put(direction.getName(), rule.save()));
    ...
    t.putInt("Pattern", pattern);
    t.putInt("Formation", formation);
    t.putInt("ParticleColor", particleColor);
    t.putBoolean("CustomAccent", customAccent);
    t.putBoolean("Sounds", sounds);
    t.putBoolean("PowerSounds", powerSounds);
    t.putBoolean("ImpactSounds", impactSounds);
    t.putBoolean("DamageSounds", damageSounds);
    t.putInt("SoundStyle", soundStyle);
    var blocking = new CompoundTag();
    var detection = new CompoundTag();
    barrierDirections
        .overrides()
        .forEach((direction, rule) -> blocking.put(direction.getName(), rule.save()));
    sensorDirections
        .overrides()
        .forEach((direction, rule) -> detection.put(direction.getName(), rule.save()));
    t.put("BarrierDirections", blocking);
    t.put("SensorDirections", detection);
  ...
  public static ControlSettings load(CompoundTag t) {
    ...
    s.sphereRadius =
        t.contains("SphereRadius")
            ? Math.max(
                SphereField.MIN_RADIUS, Math.min(SphereField.MAX_RADIUS, t.getInt("SphereRadius")))
            : SphereField.DEFAULT_RADIUS;
    ...
    for (var direction : Direction.values())
      if (damageRules.contains(direction.getName(), 10))
        s.damageDirections.set(
            direction, EntityFilter.load(damageRules.getCompound(direction.getName())));
    s.damageEnabled = t.getBoolean("DamageEnabled");
    float amount = t.contains("DamageAmount") ? t.getFloat("DamageAmount") : 2;
    s.damageAmount = Float.isFinite(amount) ? Math.max(0, Math.min(MAX_DAMAGE, amount)) : 0;
    s.damageInterval =
        t.contains("DamageInterval")
            ? Math.max(
                MIN_DAMAGE_INTERVAL, Math.min(MAX_DAMAGE_INTERVAL, t.getInt("DamageInterval")))
            : DEFAULT_DAMAGE_INTERVAL;
    ...
    s.formation =
        Math.max(
            0,
            Math.min(
                com.zeromods.core.animation.PlanarProjection.PRESET_COUNT - 1,
                t.getInt("Formation")));
    ...
    for (var direction : Direction.values()) {
      if (blocking.contains(direction.getName(), 10))
        s.barrierDirections.set(
            direction, EntityFilter.load(blocking.getCompound(direction.getName())));
      if (detection.contains(direction.getName(), 10))
        s.sensorDirections.set(
            direction, EntityFilter.load(detection.getCompound(direction.getName())));
    }
