...
public final class PlayerLookup {
  ...
  public static void register(NativeNetwork event) {
    event
        .playToServer(Request.TYPE,Request.CODEC,(p,c) -> c.enqueueWork(() -> lookup(p,(ServerPlayer)c.player())))
  ...
  private static void reply(Request r, ServerPlayer p, UUID id, String name, String error) {
    NativeNetwork.sendToPlayer(p,new Result(r.requestId,id==null?"":id.toString(),name,error));
  }
