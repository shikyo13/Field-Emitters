...
public final class FieldEffects {
  ...
  private static final MultiBufferSource.BufferSource BUFFERS =
      MultiBufferSource.immediate(new ByteBufferBuilder(BUFFER_BYTES));
  ...
  private record Exposed(VertexConsumer target, float light) implements VertexConsumer {
    ...
    @Override public VertexConsumer setColor(int red, int green, int blue, int alpha) {
      target.setColor(red, green, blue,
  ...
  public static void render(RenderLevelStageEvent event) {
    if (event.getStage() == RenderLevelStageEvent.Stage.AFTER_SKY) PENDING.clear();
    if (event.getStage() != RenderLevelStageEvent.Stage.AFTER_WEATHER || PENDING.isEmpty()) return;
    drawing = true;
