...
final class PlasmaDomeRenderer {
  ...
  private static void surfaceVertex(EmitterEntity e, VertexConsumer v, Matrix4f matrix,
    ...
    v.vertex(matrix, (float)p.x, (float)p.y, (float)p.z)
        .color((e.color >> 16 & 255)/255f, (e.color >> 8 & 255)/255f, (e.color & 255)/255f, opacity)
        .uv((float)(x * Math.PI * 2 / LONGITUDES), (float)(minimum + range * y / LATITUDES))
        .overlayCoords((Math.round(impact * 32767) << 16) | formation)
        .uv2((Math.round((e.controls.animation ? progress : 1) * 32767) << 16) | flags)
        .normal((accent >> 16 & 255)/255f, (accent >> 8 & 255)/255f, (accent & 255)/255f).endVertex();
  }
