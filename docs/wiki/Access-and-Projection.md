...
Startup uses four sweeping projection fans and takes four seconds. Collision starts when formation completes and stops immediately when power is lost; the visible shell fades out. The sphere uses the same surface presets and accent colors as the other fields, with curved impact ripples. Pattern detail uses fixed coordinates to bound rendering work as the radius grows. Tower hardware currently reuses the existing emitter materials.
