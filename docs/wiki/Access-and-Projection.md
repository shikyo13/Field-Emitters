...
Startup uses four independent laser scans that change direction, sweep in elevation and accelerate over four seconds. Short trails give the beams a soft projector glow. Only the outer shell has collision; the interior and dome floor stay open. Projection beams are visual only. Collision starts when formation completes and stops immediately when power is lost; the visible shell fades out. The sphere uses the same surface presets and accent colors as the other fields, with curved impact ripples. Pattern detail uses fixed coordinates to bound rendering work as the radius grows. Tower hardware currently reuses the existing emitter materials.
